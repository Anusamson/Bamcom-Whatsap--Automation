...
export default function Edit({ record, categories, statuses }) {
    const { data, setData, put, processing, errors } = useForm({
        title: record.title || '',
        category: record.category || 'company_information',
        content: record.content || '',
        status: record.status || 'active',
        priority: record.priority ?? 0,
        effective_date: record.effective_date ? record.effective_date.split('T')[0] : '',
        expiration_date: record.expiration_date ? record.expiration_date.split('T')[0] : '',
        keywords: Array.isArray(record.keywords) ? record.keywords.join(', ') : (record.keywords || ''),
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('knowledge.update', record.id));
    };
            ...
            header={
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400">
                                Updating #{record.id} &middot; {record.title}
                            </p>
                        </div>
                    </div>
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-rose-600 hover:text-rose-700 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg transition"
                    >
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea
                                    required
                                    rows={8}
                                    placeholder="Write detailed, factual instructions or scripts..."
                                    value={data.content}
                    ...
                    <div className="flex items-center justify-end gap-3">
                        ...
                        >
                            ...
                            {processing ? 'Saving...' : 'Update Knowledge Record'}
                        </button>
                    </div>
                </form>
            </div>

...
import { UsersRound, ArrowLeft, Edit, Users } from 'lucide-react';
import InputError from '@/Components/InputError';
import InputLabel from '@/Components/InputLabel';
import TextInput from '@/Components/TextInput';
import PrimaryButton from '@/Components/PrimaryButton';

export default function EditTeam({ team, types, eligibleLeaders, eligibleMembers }) {
    ...
    return (
        <AuthenticatedLayout header={`Edit Team: ${team.name}`}>
            <Head title={`Edit Team ${team.name} - Bamcom AI CRM`} />
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-xl font-bold text-slate-900 dark:text-white">
                                Edit Team Specifications
                            </h1>
                        </div>
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
                            Update team information, leadership, functional categorization, and team membership.
                        </p>
                    </div>
                ...
                <form onSubmit={submit} className="space-y-6">
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="md:col-span-2">
                                <InputLabel htmlFor="description" value="Team Description" />
                                <textarea
                                    id="description"
                                    rows="3"
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm space-y-4">
                        <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
                            <div>
                                <h2 className="text-sm font-bold text-slate-900 dark:text-white">Team Members</h2>
                                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                    Check or uncheck users to modify team membership.
                                </p>
                            </div>
                            <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300">
                                {data.member_ids.length} members
                            </span>
                        </div>
